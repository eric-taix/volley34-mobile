...
abstract class ClubStatsState extends Equatable {
  ...
  const ClubStatsState({this.wonMatches = 0, this.totalMatches = 0})
      : assert(wonMatches <= totalMatches);
  ...
  Stream<ClubStatsState> mapEventToState(ClubStatsEvent event) async* {
    if (event is ClubStatsLoadEvent) {
      ...
      var stats = teamsResults.fold(Tuple2<int, int>(0, 0), (acc, matchResult) {
        ...
        bool hostedByClubTeam = (teams.firstWhere(
                (team) => team.code == matchResult?.hostTeamCode,
                orElse: () => null) !=
            null);
        if (hostedByClubTeam ?? false) {
          acc = matchResult.totalSetsHost > matchResult.totalSetsVisitor
              ? acc.withItem1(acc.item1 + 1)
              : acc;
        } else {
          acc = (matchResult?.totalSetsVisitor ?? 0) >
                  (matchResult?.totalSetsHost ?? 0)
              ? acc.withItem1(acc.item1 + 1)
              : acc;
        }
        return acc;

...
class TeamProvider {
  Future<List<Team>> loadClubTeams(String? clubCode) async {
    Response response =
        await dio.get("/clubs/$clubCode/equipes", options: buildConfigurableCacheOptions()).catchError((error) {
  ...
  Future<List<ClassificationSynthesis>> loadClassificationSynthesis(String? teamCode) async {
    Response response = await dio.get("/Classements/equipes/$teamCode", options: buildConfigurableCacheOptions());
    if (response.statusCode == 200) {
  ...
  Future<Club> loadTeamClub(String? teamCode) async {
    Response response = await dio.get("/equipes/$teamCode/club", options: buildConfigurableCacheOptions());
    if (response.statusCode == 200) {
